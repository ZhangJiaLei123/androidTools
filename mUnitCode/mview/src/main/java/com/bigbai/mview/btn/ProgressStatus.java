...
public class ProgressStatus implements ButtonStatus {
    ...
    public ProgressStatus(CommitButton button, int width, int height, int centerX, int centerY) {
        this.button = button;
        this.width = width;
        this.height = height;
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                if(progress >= Andle_Full){
                    //如果是手动结束模式
                    if(button.getMode() == CommitButton.Mode.Hand_Finish && button.isProgressStop()){
                        //改变状态
                        ...
                        if(status == Status_Once){
                            ...
                            if(button.getMode() == CommitButton.Mode.Auto_Finish){
                                //改变状态
                                button.changeStatus(Status.Finish, width, height, centerX, centerY);
                                return;
                            }else{
